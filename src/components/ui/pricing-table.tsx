...
export interface PricingTableProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  onGetStarted?: (plan: PlanLevel, interval: BillingInterval) => void
...
export function PricingTable({
  ...
  const [interval, setInterval] = React.useState<BillingInterval>(defaultInterval)
  ...
  const selectedPlanName = plans.find((p) => p.level === selectedPlan)?.name ?? "plan"

  return (
    <section
      className={cn(
        "bg-background text-foreground py-12 sm:py-16 md:py-20 px-4",
        className,
      )}
    >
      <div
        className={cn("w-full max-w-5xl mx-auto px-0 sm:px-4", containerClassName)}
        {...props}
      >
        <div className="flex justify-end mb-4 sm:mb-8">
          <div className="inline-flex items-center gap-2 text-xs sm:text-sm bg-muted rounded-lg p-1">
            <button
              type="button"
              onClick={() => handleIntervalChange("monthly")}
              className={cn(
                "px-3 py-1 rounded-md transition-colors",
                interval === "monthly" ? "bg-zinc-100 dark:bg-zinc-800" : "text-zinc-500",
              )}
            >
              Monthly
            </button>
            <button
              ...
              className={cn(
                "px-3 py-1 rounded-md transition-colors",
                interval === "quarterly" ? "bg-zinc-100 dark:bg-zinc-800" : "text-zinc-500",
              )}
            >
              Quarterly
            </button>
            <button
              ...
              className={cn(
                "px-3 py-1 rounded-md transition-colors",
                interval === "yearly" ? "bg-zinc-100 dark:bg-zinc-800" : "text-zinc-500",
              )}
            >
              Yearly
            </button>
            ...
            >
              ...
              <div className="flex items-baseline gap-1">
                ...
                <span className="text-sm font-normal text-zinc-500">
                  /{interval === "yearly" ? "year" : interval === "quarterly" ? "quarter" : "month"}
                </span>
              </div>
            </button>
          ))}
        ...
        <div className="mt-8 text-center">
          <Button
            ...
            onClick={() => onGetStarted?.(selectedPlan, interval)}
            className={cn("w-full sm:w-auto bg-blue-500 hover:bg-blue-600 px-8 py-2 rounded-xl", buttonClassName)}
          >
            Subscribe
...
function shouldShowCheck(
  ...
  const includedIndex = plans.findIndex((plan) => plan.level === included)
  const currentIndex = plans.findIndex((plan) => plan.level === level)
  if (includedIndex === -1 || currentIndex === -1) return false

  return currentIndex >= includedIndex
}
